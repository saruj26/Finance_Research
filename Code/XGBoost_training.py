...
INPUT_FILE = os.path.join(PROJECT_PATH, "Data", "XGBoost_final_dataset.csv")
...
def print_metrics(name, y_true, pred, proba):
    ...
    print(f"\n{name} Accuracy: {acc:.4f} | LogLoss: {ll:.4f}")
...
plt.plot(epochs, val_logloss, label="Val logloss")
...
plt.legend()
plt.grid(alpha=0.3)
plt.savefig(os.path.join(OUTPUT_DIR, "logloss_curve.png"), dpi=300, bbox_inches="tight")
plt.show()
